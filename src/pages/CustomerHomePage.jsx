...
const CustomerHomePage = () => {
  ...
  const productImages = {
    'Supiri Keeri Samba': 'src/assets/SupiriKeeriSamba.jpg',
    'Supiri Samba Rice': 'src/assets/SupiriSambaRice.jpg',
    'Supiri Nadu Rice': 'src/assets/SupiriNaduRice.jpg',
    'Supiri Red Nadu Rice': 'src/assets/SupiriRedNaduRice.jpg',
    'Supiri White Raw Rice': 'src/assets/SupiriWhiteRawRice.jpg',
    'Supiri Samba Raw Rice': 'src/assets/SupiriSambaRawRice.jpg',
    'Supiri Red Raw Rice': 'src/assets/SupiriRedRawRice.jpg',
  };
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Map image paths to fetched products
        const productsWithImages = data.map(product => ({
          ...product,
          image: productImages[product.name] || 'src/assets/placeholder.jpg',
        }));
  ...
  for (let i = 0; i < products.length; i += 3) {
    productSets.push(products.slice(i, i + 3));
  }
        ...
        {productSets.length > 0 ? (
          <div className="cust-home-product-carousel">
            ...
            <div className="cust-home-product-grid">
              {productSets[currentSet].map((product, index) => (
                <div className="cust-home-product-card" key={index}>
                  <img src={product.image} alt={product.name} onError={(e) => { e.target.src = 'src/assets/placeholder.jpg'; console.log('Image not found, using placeholder'); }} />
                  <h4>{product.name}</h4>
                  <p>Rs.{product.price}/kg (Stock: {product.stock}kg)</p>
                </div>
              ))}
            </div>
            <button className="cust-home-carousel-arrow cust-home-arrow-right" onClick={handleNextSet}>
